<div id="block-answer-ticket">
	<div id="block-thread-subject">
		...
		<div id="title-threads-uid"><?php echo t("t_posted")." ".$ticket[0]->uid_users; ?></div>
	</div>
	<div id="block-list-answer">
		<?php foreach ($answer as $row): ?>
			<div class="block-this-answer" value="<?php echo $row->id; ?>">
				...
				<div class="text-uid"><?php echo t("t_posted")." ".$row->uid_users; ?></div>
			</div>
		<?php endforeach; ?>
	</div>

	<div id="title-add-answer"><?php echo t("t_answer"); ?></div>
	<?php echo form_open('tickets/add_answertickets'); ?>
	<input type="hidden" id="input-id-ticket" name="id" value="<?php echo $ticket[0]->id; ?>" />
	<textarea id="textarea-answer-thread" name="answer" rows="10" cols="86" placeholder="<?php echo t("t_writemessage"); ?>"></textarea>
    <br/>
	<?php if ($ticket[0]->etat == 0): ?>
		<input type="submit" value="<?php echo t("t_add"); ?>" id="submit-add-answer"/>
	<?php else: ?>
		<div id="block-error-subject-forum"><?php echo t("t_autorize"); ?></div>
	<?php endif; ?>
	<?php echo form_close(); ?>
</div>